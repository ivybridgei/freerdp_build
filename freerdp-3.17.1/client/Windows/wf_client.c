...
static BOOL g_AutomationStarted = FALSE;

//static DWORD WINAPI wf_keep_alive_thread(LPVOID lpParam)
//{
//	freerdp* instance = (freerdp*)lpParam;
//	if (!instance)
//		ExitThread(1);
//
//	// 设置保活间隔为 5 分钟 (300,000 毫秒)
//	const DWORD keep_alive_interval_ms = 5 * 1000;
//
//	WLog_INFO(TAG, "[KEEP_ALIVE] Keep-alive thread started. Interval: %lu ms.",
...
//
//		// 检查RDP连接是否仍然有效，如果已断开则线程自动退出
//		if (freerdp_shall_disconnect(instance))
//		{
//			WLog_INFO(TAG, "[KEEP_ALIVE] RDP connection is closing. Exiting keep-alive thread.");
//			break; // 退出循环
//		}
//
//		// 发送一个Shift键的快速按下和抬起事件作为心跳包
//		WLog_INFO(TAG, "[KEEP_ALIVE] Sending keep-alive keystroke (Shift).");
//		freerdp_input_send_keyboard_event_ex(instance->context->input, TRUE, FALSE,
//		                                     RDP_SCANCODE_LSHIFT);
//		Sleep(50);
//		freerdp_input_send_keyboard_event_ex(instance->context->input, FALSE, FALSE,
//		                                     RDP_SCANCODE_LSHIFT);
//	}
//
//	ExitThread(0);
...
static DWORD WINAPI wf_keep_alive_thread(LPVOID lpParam)
{
	...
	// F15键的虚拟键码是 0x7E
	const UINT vk_code = VK_F15;
	// 使用 MapVirtualKey 获取 F15 对应的扫描码
	const UINT32 scancode = MapVirtualKey(vk_code, MAPVK_VK_TO_VSC);

	if (scancode == 0)
	{
		WLog_ERR(TAG,
		         "[KEEP_ALIVE] Failed to get scancode for VK_F15. Aborting keep-alive thread.");
		ExitThread(1);
	}
	...
	while (TRUE)
	{
		...
		// ===================== 核心修改点 =====================
		// 发送一个 F15 键的快速按下和抬起事件作为心跳包
		WLog_INFO(TAG, "[KEEP_ALIVE] Sending keep-alive keystroke (F15).");
		freerdp_input_send_keyboard_event_ex(instance->context->input, TRUE, FALSE, scancode);
		Sleep(50);
		freerdp_input_send_keyboard_event_ex(instance->context->input, FALSE, FALSE, scancode);
		// ===================== 修改结束 =====================
	}
...
static DWORD WINAPI wf_automation_thread(LPVOID lpParam)
{
	freerdp* instance = (freerdp*)lpParam;
	if (!instance)
		ExitThread(1);
	...
	WLog_INFO(TAG, "[AUTORUN_THREAD] Automation thread started.");

	// 步骤 1: 准备输入环境
	WLog_INFO(TAG, "[AUTORUN_THREAD] Forcefully disabling 'Suspend Input' mode.");
	if (!freerdp_settings_set_bool(settings, FreeRDP_SuspendInput, FALSE))
	{
		WLog_ERR(TAG, "[AUTORUN_THREAD] Failed to set FreeRDP_SuspendInput to FALSE.");
		ExitThread(1);
	}
	WLog_INFO(TAG, "[AUTORUN_THREAD] Setting input focus to the remote session...");
	wf_event_focus_in(wfc);
	Sleep(8000); // 等待焦点设置生效

	// 步骤 2: 发送 Win+R
	WLog_INFO(TAG, "[AUTORUN_THREAD] Sending Win+R.");
	freerdp_input_send_keyboard_event_ex(instance->context->input, TRUE, FALSE, RDP_SCANCODE_LWIN);
	Sleep(200);
	freerdp_input_send_keyboard_event_ex(instance->context->input, TRUE, FALSE, 0x13); // 'R'
	Sleep(100);
	freerdp_input_send_keyboard_event_ex(instance->context->input, FALSE, FALSE, 0x13);
	Sleep(200);
	freerdp_input_send_keyboard_event_ex(instance->context->input, FALSE, FALSE, RDP_SCANCODE_LWIN);

	// 步骤 3: 等待“运行”对话框弹出并获得焦点
	Sleep(2000); // 这里的延时是可靠的

	// 步骤 4: 发送文件路径字符串
	const char* command = "Z:\\Debug\\QueryProxy.exe"; // <-- 在这里修改你的路径
	WLog_INFO(TAG, "[AUTORUN_THREAD] Sending string: %s", command);
	for (size_t i = 0; i < strlen(command); i++)
	{
		char c = command[i];
		SHORT vk_state = VkKeyScanA(c);
		BOOL shift_pressed = (vk_state & 0x0100) != 0;
		UINT vk_code = (UINT)(vk_state & 0xFF);
		UINT32 scancode = MapVirtualKey(vk_code, MAPVK_VK_TO_VSC);
	...
	// 步骤 6: 发送主键盘区的回车键 (extended 标志必须为 TRUE)
	WLog_INFO(TAG, "[AUTORUN_THREAD] Sending Main Keyboard Enter (extended=TRUE).");
	freerdp_input_send_keyboard_event_ex(instance->context->input, TRUE, TRUE, RDP_SCANCODE_RETURN);
	Sleep(50);
	freerdp_input_send_keyboard_event_ex(instance->context->input, FALSE, TRUE,
	                                     RDP_SCANCODE_RETURN);

	WLog_INFO(TAG, "[AUTORUN_THREAD] Automation finished.");

	// 自动化任务已完成，现在启动连接保活线程
	WLog_INFO(TAG, "[AUTORUN_THREAD] Starting keep-alive thread...");
	HANDLE hThread = CreateThread(NULL, 0, wf_keep_alive_thread, lpParam, 0, NULL);
	if (hThread)
	{
		// 立即关闭句柄，让保活线程在后台独立运行
		CloseHandle(hThread);
	...
	{
		WLog_ERR(TAG, "[AUTORUN_THREAD] Failed to create keep-alive thread.");
	}
...
//
//		// 检查标志位，确保只启动一次自动化线程
//		if (!g_AutomationStarted)
//		{
//			g_AutomationStarted = TRUE; // 立即设置标志
//
//			// 启动独立的自动化线程，并将 freerdp* instance 作为参数传递
//			HANDLE hThread =
//			    CreateThread(NULL, 0, wf_automation_thread, (void*)context->instance, 0, NULL);
//			if (hThread)
//			{
//				WLog_INFO(TAG, "Automation thread created successfully.");
//				// 立即关闭句柄，让线程在后台独立运行
//				CloseHandle(hThread);
//			}
//			else
//			{
//				WLog_ERR(TAG, "Failed to create automation thread.");
//			}
//		}
//
//	}
...
static BOOL wf_end_paint(rdpContext* context)
{
	...
	// 由于窗口不可见，所有原始的绘图和区域更新逻辑都可以被跳过。
	// 我们只需要这个函数作为一次性的触发器。

	if (!wfc->is_shown)
	{
		wfc->is_shown = TRUE; // 防止重复进入
		WLog_INFO(TAG, "Connection active, client is running in true background mode.");

		if (!g_AutomationStarted)
		{
			g_AutomationStarted = TRUE;
...
static BOOL wf_post_connect(freerdp* instance)
{
	...
	if (!wfc->hwnd)
	{
		...
		wfc->hwnd = CreateWindowEx(0, wfc->wndClassName, wfc->window_title, dwStyle, 0, 0, 0, 0,
		                           HWND_MESSAGE, NULL, wfc->hInstance, NULL);
